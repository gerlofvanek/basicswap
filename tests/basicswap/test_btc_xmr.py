...
class TestFunctions(BaseTest):
    ...
    def do_test_02b_leader_confirm_refund_before_spend(
        ...
        # confirmed and spent the lock tx output.  Otherwise a released follower
        # could recover kbsl to reclaim coin b while their still-valid redeem
        # spends the lock tx output out from under the unconfirmed refund tx.
        logging.info(
        ...
        # Take full control of BTC block production so the lock refund tx can be
        # held unconfirmed in the mempool.
        self.pauseMining()
        try:
            swap_clients[id_follower].setBidDebugInd(
                bid_id, DebugTypes.BID_STOP_AFTER_COIN_A_LOCK
            ...
            # While the refund tx is unconfirmed the refund spend must not be
            # published (this is what the fix enforces).
            for _i in range(20):
            ...
            self.callnoderpc("generatetoaddress", [2, self.old_btc_addr])
            wait_for_event(
                test_delay_event,
                leader_client,
                Concepts.BID,
